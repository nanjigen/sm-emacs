...
**** NEXT Status bar
:PROPERTIES:
:CREATED:  [2023-02-17 Fri 16:59]
:ID:       2a9d0aef-3dcd-406f-ac27-77311fc85f99
:TRIGGER:  chain-find-next(NEXT,from-current,priority-up,effort-down)
:END:

Noticeably missing from the core window list is the [[https://help.supermemo.org/wiki/Status_bar][status bar]]. Due to some limitations in EXWM (and my own knowledge), its size and transience make it difficult to place satisfactorily below the statistics window.
It's =exwm-title= variable comes up as =nil=, so we'll have to test =exwm-title= against an empty/nil string?

#+begin_src elisp :noweb yes :noweb-ref no
(add-hook 'exwm-update-title-hook
          (defun sm-status-bar-title-hook ()
            "Manage the status bar name"
            (when (= exwm-title nil) ;; FIXME this doesn't work
              (exwm-workspace-rename-buffer "sm-status-bar")
              (setq exwm-title "sm-status-bar"))))
#+end_src
...
**** floating window alist
:PROPERTIES:
:CREATED:  [2023-02-21 Tue 15:26]
:ID:       c0710ccc-602a-4223-b40c-349f4104ef15
:END:

The 'floating windows' are made up of mostly transient windows, often used for confirming an action or making a selection of a subset or registry.

Due to the rather egregious frame redraws that occur when a core (tiled) window is replaced or resized, I have chosen to make these windows floating. This prevents, say, the element window being replaced by another tiled window and greatly speeds up the general operation of Supermemo as we aren't waiting for visible redraws to complete. Also, it allows us to keep context, as we can often still see some of all the windows behind the floating one, informing our decisions regarding the prompt before us.

#+begin_src elisp :noweb yes :noweb-ref sm-window-alists
(setq sm-exwm-floating-buffer-alist
      '(("sm-ancestors" . "Ancestors")
        ("sm-choice-window" . "Choices")
...
**** Floating naming hook
:PROPERTIES:
:CREATED:  [2023-02-21 Tue 15:29]
:ID:       df122f72-5633-4826-a1de-bee1b259b82b
:END:

As with the =core-window-title-hook=, we iterate via a ~cl-loop~. However you'll notice below an =exwm-manage-finish-hook=.
...
(add-hook 'exwm-update-title-hook
          (defun sm-exwm-floating-window-title-hook ()
            ...
            (cl-loop for (key . value) in sm-exwm-floating-buffer-alist
                     ...
                     do (exwm-workspace-rename-buffer key)
                     (setq exwm-title key)
                     ;; (exwm-floating--set-floating (exwm--buffer->id (get-buffer key))) ;; FIXME doesn't work
                     )))

#+end_src

This hook runs after ~exwm~ manages the =sm18.exe= class and sets floating for the floating window set each time a new window is spawned in SM. This is due to an issue related to managing the ~sm-element-window~, discussed later on:

#+begin_src elisp :noweb yes :noweb-ref sm-window hooks
(add-hook 'exwm-manage-finish-hook
          (defun sm-exwm-set-floating-window-hook ()
            "Iterate over floating alist and make `exwm-title' floating"
            (cl-loop for (key . value) in sm-exwm-floating-buffer-alist
                     if (string= exwm-title key)
                     do (exwm-floating--set-floating (exwm--buffer->id (get-buffer key))))))
#+end_src
...
(add-hook 'exwm-manage-finish-hook
          (lambda ()
            (cl-loop for (key . value) in sm-exwm-core-buffer-alist
                     if `((string= (current-buffer) ,key))
                     if  exwm--floating-frame
                     do (with-current-buffer key
                          (exwm-floating--unset-floating exwm--id)))))
#+end_src

For some reference about the above functions see issue [[https://github.com/ch11ng/exwm/issues/516][#516]].
